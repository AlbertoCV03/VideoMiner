...
public class VideoController {
    ...
    @Operation(summary = "Get all videos from a channel", description = "Get all the videos from a specified channel")
    ...
    @Operation(summary = "Get all videos", description = "Get all the videos from the database")
    @GetMapping("/videos")
    public List<Video> getAllVideos(
            ...
            @Parameter(description = "Order of the videos (use 'date' to order by release date descending)") @RequestParam(required = false) String order,
        ...
        if ("date".equals(order)) {
            sort = Sort.by("releaseTime").descending();
        }
            ...
            for (Video video : paginados) {
                ...
                if (findDate == null || findDate.equals("exact")) {
                    if (date.getYear() == year && month == null && day == null) {
                        res.add(video);
                    } else if (date.getYear() == year && month != null && date.getMonthValue() == month
                            && day == null) {
                        res.add(video);
    ...
    @Operation(summary = "Get a video", description = "Get the specified video from the database")
    ...
    @Operation(summary = "Update a video", description = "Update the specified video from the database")
    ...
    @Operation(summary = "Delete a video", description = "Delete the specified video from the database")
    @ResponseStatus(HttpStatus.NO_CONTENT)
